...
# we apply trans_conv3d - acti - dropout - norm -
...
class Decoder_vqvae_conv(nn.Module):
    """
    ...
    def __init__(self, in_dim, h_dim, n_res_layers=2, res_h_dim=256):
        super(Decoder_vqvae_conv, self).__init__()
        kernel = 4
        stride = 2
        self.conv_stack = nn.Sequential(
            ResidualStack(h_dim, h_dim, res_h_dim, n_res_layers),
            trans_conv3d_adn(h_dim, h_dim, kernel_size=kernel, stride=stride, padding=1),
            trans_conv3d_adn(h_dim, h_dim // 2, kernel_size=kernel, stride=stride, padding=1),
            trans_conv3d_adn(h_dim // 2, in_dim, kernel_size=kernel, stride=stride, padding=1)
        )
