...
class EqualConv3d(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, stride=1, padding=0, bias=True):
        ...
        self.scale = 1 / sqrt(in_channel * kernel_size ** 3)
...
class PatchDiscriminator(nn.Module):
    ...
    """
    def __init__(self, input_nc: int = 1, ndf: int = 64, n_layers: int = 3, use_actnorm: bool = False) -> None:
...
class StyleDiscriminator(nn.Module):
    def __init__(self, size=64, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
        ...
        blocks = [ConvLayer(1, channels[size], 1)]
