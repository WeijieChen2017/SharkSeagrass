...
class Sam3D(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(self.channel_list)-1):
            ...
            # 16 -> 128 is 3x, so there are 3 conv layers
            level_conv = nn.Sequential(
                Convolution(
                    spatial_dims=3,
                ...
                ResidualUnit(
                    spatial_dims=3,
                    in_channels=self.channel_list[i+1],
                    out_channels=self.channel_list[i+1],
                    subunits=3,
                    act=Act.PRELU,
                    norm=Norm.INSTANCE,
            ...
            self.out_conv.append(level_conv)
    ...
    def load_pretrain(self, path) -> None:
        pretrain_model = torch.load(path, map_location="cpu")["model_state_dict"]
        # there are only image_encoder in the pretrain model
        self.image_encoder.load_state_dict(pretrain_model, strict=False)
        print("Pretrained model loaded from", path)
